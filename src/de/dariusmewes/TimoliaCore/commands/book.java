...
public class book extends TCommand {
	...
	public void perform(CommandSender sender, String[] args) {
		...
		if (args[0].equalsIgnoreCase("load")) {
			try {
				File file = new File(TimoliaCore.dataFolder + File.separator + "books" + File.separator + TimoliaCore.getCorrectName(name) + ".txt");
				if (!file.exists()) {
			...
			try {
				File file = new File(TimoliaCore.dataFolder + File.separator + "books" + File.separator + name + ".txt");
				if (file.exists())
					file.delete();
